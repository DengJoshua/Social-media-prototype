...
const Post = ({ post, user, likePost, unlikePost }) => {
  return (
    <div className="section mb-3" key={post._id}>
      ...
      <section className="post-text mb-2">
        <p>{post.text}</p>
      </section>
      {post.image !== "" ? (
        <section className="post-media mb-1">
      ...
      <footer className="post-stats mt-2">
        {getLikeClasses(post)}
        <Link to={`/me/home/posts/${post._id}`} className="comment-link">
          <i className={getCommentClasses(post)}> {post.comments.length}</i>
        </Link>
        <i className="fa fa-share"> {post.shares.length}</i>
        <Link to={`/me/home/posts/${post._id}`} className="float-right">
          Go to post.
        </Link>
      </footer>
    </div>
  ...
  function getLikeClasses(post) {
    ...
    for (let i = 0; i < postlen; i++) {
      ...
      if (liker._id.toString() === user._id.toString()) {
        return (
          <i
            className="fa fa-thumbs-o-up text-danger"
    ...
    return (
      <i className="fa fa-thumbs-o-up" onClick={() => likePost(post._id)}>
        {" "}
        {post.likes.length}
      </i>
    );
  ...
  function checkIsMine(post) {
    if (user._id.toString() === post.user._id.toString()) {
      return (
        <Icon icon="ellipsis-h" style={{ fontSize: "30px" }} className="h-0" />
      );
    }
    return "";
  }
