        ...
        <div className="suggestions">
          ...
          <div>
            <div className="suggestion mb-3 ">
              <main>
                <img
                  src={require("../../images/profile_girl.jpg")}
              ...
              <Button color="violet" className="h-50">
                Follow
              </Button>
            </div>
            <div className="suggestion mb-3">
              ...
              <Button appearance="ghost" color="blue" className="h-50">
                Followed
              </Button>
            </div>
            <div className="suggestion">
              <main>
                <img
                  src={require("../../images/people1.jpg")}
                  className="profile-pic"
                  alt=""
                />
                <section className="ml-2">
            ...
            <React.Fragment>
              ...
              <div className="post">
                <div className="post-content">
                  <div className="w-50">
                    {user.recent.image && (
                      <img src={user.recent.image} alt="" />
                    )}
                    {user.recent.video && (
                      <video src={user.recent.video} controls></video>
                    )}
                    {user.recent.text && (
                      <p className="post-text">{user.recent.text}</p>
                    )}
                  </div>
                  <div className="ratings">
                    <p className="date">
                      Published: {this.formatDate(user.recent.createdAt)}
                    </p>
                    <section>
                      <Icon
                        icon="commenting-o p-2"
                        className={this.getCommentClasses(user.recent)}
                      >
                        {" "}
                        {user.recent.comments.length}
                      </Icon>
  ...
  getLikeClasses(post) {
    ...
    for (let i = 0; i < postlen; i++) {
      ...
      if (liker._id.toString() === user._id.toString()) {
        return (
          <i className="fa fa-thumbs-o-up text-danger"> {post.likes.length}</i>
        );
      }
    }
    return <i className="fa fa-thumbs-o-up"> {post.likes.length}</i>;
  }
