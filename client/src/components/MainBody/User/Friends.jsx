...
const Friends = ({ history, user, unfriend, cancelfriendRequest }) => {
  return (user.friends.length === 0 ? <div style={{ padding: "10px" }} ><i className="fa fa-arrow-left goback" onClick={() => history.goBack()} /> <p>You currently have no friends.</p></div> :
    <div style={{ padding: "10px" }}>
      <i className="fa fa-arrow-left goback" onClick={() => history.goBack()} />
      {user.friends.map(friend => friend.status === "pending" ?
