...
import { getPosts, savePost, likePost, unlikePost, saveComment } from '../../services/postService'
...
class NewsFeed extends Component {
    ...
    LikePost = async (id) => {
        await likePost(this.props.user._id, id)
        const { data: posts} = await getPosts()
        this.setState({ posts })
    }

    unLikePost = async (id) => {
        await unlikePost(this.props.user._id, id)
        const { data: posts} = await getPosts()
        this.setState({ posts })
    }

    postComment = async (id, comment) => {
        await saveComment(this.props.user._id, id, comment)
    ...
    render() {
        ...
        return (
            ...
            <br/>
            <Posts posts={posts} likePost={this.LikePost} unlikePost={this.unLikePost}
            postComment={this.postComment} user={user}
            />
            </div>
        )
    }
